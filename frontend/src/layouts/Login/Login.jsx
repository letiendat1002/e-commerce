...
const Login = () => {
  ...
  const handleRegister = async (values) => {
    ...
    if (res.payload.status === 400) {
      if (
        res.payload.message == `Phone {${res.meta.arg.phone}} is already taken`
      ) {
        toast.error(`Số điện thoại đăng ký đã tồn tại!`);
      } else if (
        res.payload.message == `Email {${res.meta.arg.email}} is already taken`
      ) {
  ...
  const handleCloseResetForm = () => {
    ...
    if (
      ...
      forgetForm.classList.contains("d-none")
    ) {
    ...
    <div id="login">
      <div className="login__container">
        ...
        >
          ...
          >
            ...
            <div className="login__left--account">
              ...
              <Link onClick={() => handleLogin("login")} className="logins">
                <span>Đăng ký</span>
              </Link>
            </div>
          </Form>
        ...
        >
          ...
          <div className="login__left--account" style={{ padding: "20px 0" }}>
            <span>Bạn đã có tài khoản?</span>
            <Link onClick={() => handleLogin("signin")} className="signin">
              <span>Đăng Nhập</span>
            </Link>
          </div>
        </div>
